...
class UtilityRepositoryImpl(
    ...
    override fun getLandingSpots(map: String, utility: String) = flow {
        try {
            val landingSpotsRef = mapsRef.document(map).collection(utility)
    ...
    override fun getThrowingSpots(map: String, utility: String, landingSpot: String) = flow {
        try {
            val throwingSpotsRef = mapsRef.document(map).collection(utility).document(landingSpot).collection(Constants.THROW)
